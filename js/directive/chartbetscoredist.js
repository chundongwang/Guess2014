...
WorldCupApp.getModule().directive('gwChartbetscoredist', ['Miner', function(Miner) {
  ...
  function categroize(betscores) {
    var data = betscores.reduce(function(prev, betscore) {
      var rightAboutScore = betscore[1].rightAboutScore;
      if (!prev[rightAboutScore]) {
        prev[rightAboutScore] = 1;
      } else {
        prev[rightAboutScore]++;
      }
      return prev;
    }, []);
  ...
  return {
    ...
    templateUrl: 'js/directive/chartbetscoredist.tpl.html',
    link: function(scope, elem, attrs) {
      scope.$watch('betscores', function(newVal, oldVal) {
        ...
        var ctx = document.getElementById("gwChartBetScoreDist").getContext("2d");
